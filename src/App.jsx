...
function App() {
  const categories = ['All', 'Work', 'Personal', 'Study', 'Shopping'];
      ...
      <div className={`w-full max-w-full sm:max-w-xl mx-auto ${boxBgClass} p-3 sm:p-10 rounded-2xl sm:rounded-3xl shadow-[0_8px_32px_0_rgba(31,38,135,0.37)] ${borderClass} transition-all duration-300 hover:shadow-[0_16px_48px_0_rgba(31,38,135,0.37)] mt-0 sm:mt-8`}>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-sm font-medium ${selectedCategory === cat ? 'bg-purple-500 text-white' : 'bg-gray-200 text-gray-800'}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="flex flex-col sm:flex-row mb-4 gap-2 sm:gap-3">
          ...
          <select
            ...
            className={`rounded-lg px-2 py-2 text-sm ${theme === 'light' ? 'bg-white text-gray-900' : 'bg-gray-800 text-white'}`}
          >
        ...
        <ToDoList
          todos={tasks.filter(todo => selectedCategory === 'All' || (todo.category && todo.category === selectedCategory))}
